...
$sqlinsert = "SELECT * FROM tbl_carts WHERE customer_email = '$email' AND subject_id = '$subjectId' AND cart_status IS NULL";
...
if ($number_of_result > 0) {
	...
	$cartqty = $cartqty + 1;
	$updatecart = "UPDATE `tbl_carts` SET `cart_qty`= '$cartqty' WHERE customer_email = '$email' AND subject_id = '$subjectId' AND cart_status IS NULL";
	$conn->query($updatecart);
} 
else 
{
    $addcart = "INSERT INTO `tbl_carts`(`customer_email`, `subject_id`, `cart_qty`) VALUES ('$email','$subjectId','$cartqty')";
