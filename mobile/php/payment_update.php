...
$signed= hash_hmac('sha256', $signing, 'S-GBV6Ezjop8c09zXuGnvv0w');
if ($signed === $data['x_signature']) {
    if ($paidstatus == "Success"){ //payment success
        ...
        $sqlupdatecart = "UPDATE `tbl_carts` SET `cart_status`='paid',`receipt_id`='$receiptid' WHERE customer_email='$email' AND cart_status IS NULL";
        if ($conn->query($sqlinsertpayment) && $conn->query($sqlupdatecart)){
            ...
            if ($result->num_rows > 0) {
                while ($rows = $result->fetch_assoc()) {
                    ...
                    $subjectsessions = $rows['cart_qty'];
                    $updatesubjectsqty = "UPDATE tbl_subjects set subject_sessions = (subject_sessions-$subjectsessions) WHERE subject_id = '$subjectId'";
